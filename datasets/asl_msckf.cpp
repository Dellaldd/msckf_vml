...
{
  ros::init(argc, argv, "image_listener_vml");
  ros::NodeHandle nh;
  ...
  ros::Publisher odom_pub = nh.advertise<nav_msgs::Odometry>("odom_vml", 100);
  ros::Publisher map_pub = nh.advertise<sensor_msgs::PointCloud2>("map_vml", 100);
  ros::Publisher cam_pose_pub = nh.advertise<geometry_msgs::PoseArray>("cam_state_poses_vml", 100);
  ros::Publisher pruned_cam_states_track_pub = nh.advertise<nav_msgs::Path>("pruned_cam_states_path_vml", 100);
  ros::Publisher cam_state_pub = nh.advertise<msckf_mono::CamStates>("cam_states_vml", 100);

  ros::Publisher imu_track_pub = nh.advertise<nav_msgs::Path>("imu_path_vml", 100);
  nav_msgs::Path imu_path;

  ros::Publisher gt_track_pub = nh.advertise<nav_msgs::Path>("ground_truth_path_vml", 100);
  nav_msgs::Path gt_path;

  ros::Publisher time_state_pub = nh.advertise<msckf_mono::StageTiming>("stage_timing_vml",10);

  image_transport::ImageTransport it(nh);
  image_transport::Publisher raw_img_pub = it.advertise("image_vml", 1);
  image_transport::Publisher track_img_pub = it.advertise("image_track_vml", 1);
    ...
    if(imu_reading){
      ...
      if(std::get<1>(data_pack)){
        ...
        if(elapsed_clock_time > elapsed_dataset_time){
          ...
          msckf.augmentState(state_k, (imu0->get_time())/1e9);
